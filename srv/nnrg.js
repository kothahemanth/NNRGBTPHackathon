...
module.exports = cds.service.impl(async function () {
    const { States, Business } = this.entities;
    this.on("READ", Business, async (req) => {
        const results = await cds.run(req.query);
        return results;
      });
    this.before("CREATE",  Business, async (req) => {
        ...
        const query1 = SELECT.from( Business).where({ bp_number: req.data.bp_number });
